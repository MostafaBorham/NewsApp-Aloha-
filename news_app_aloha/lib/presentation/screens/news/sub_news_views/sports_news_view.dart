...
class SportsNewsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<NewsBloc>(
      create: (context) =>AppInjector.instance<NewsBloc>()..add(GetAllNewsEvent(newsType: NewsCategories.sports)),
      child: BlocBuilder<NewsBloc, NewsState>(
        builder: (context, state) {
          return RefreshIndicator(
            onRefresh: ()async{
              NewsBloc.getInstance(context).add(GetAllNewsEvent(newsType: NewsCategories.sports));
            },
            child: ConditionalBuilder(
              condition: state is! LoadingAllNewsState,
              builder: (context) =>
                  ConditionalBuilder(
                    condition: NewsBloc.getInstance(context).sportsNewsList.isNotEmpty,
                    builder: (context) {
                      return NewsList(newsList: NewsBloc.getInstance(context).sportsNewsList,);
                    },
                    fallback: (context) =>
                        NotFoundComponent(message: state is FailGetAllNewsState
                            ? state.failMessage
